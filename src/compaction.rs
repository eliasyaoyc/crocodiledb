...
pub struct Compaction<F: File, C: Comparator> {
    ...
    // This field should be access via CompactionInputRelation
    // and the files of level n and level n + 1 are all stored.
...
impl<O: File, C: Comparator + 'static> Compaction<O, C> {
    ...
    pub fn should_stop_before(&mut self, ikey: &[u8], icmp: &InternalKeyComparator<C>) -> bool {
        ...
        while self.grand_parent_index < self.grand_parents.len()
            && icmp.compare(ikey, self.grand_parents[self.grand_parent_index].largest.data()) == Ordering::Greater
        {
            if self.seen_key {
    ...
    /// Reports whether it is guaranteed that there are no key/value pairs
    /// at c.level + 2 or higher that have the user key.
    pub fn key_exist_in_deeper_level(&mut self, user_key: &[u8]) -> bool {
        let v = self.input_version.as_ref().unwrap();
        let ucmp = &self.input_version.as_ref().unwrap().comparator().comparator;
            ...
            for level in self.level + 2..max_levels {
                for f in v.get_level_files(level) {
                    if ucmp.compare(user_key, f.largest.user_key()) != Ordering::Greater {
                        ...
                        // For level above level 0, the files within a level are in
    ...
    /// Apply deletion for current inputs and current files to the edit.
    pub fn apply_to_edit(&mut self) {
        ...
        for output in self.outputs.drain(..) {
            self.edit.file_delta.new_files.push((self.level + 1, output))
        }
    }

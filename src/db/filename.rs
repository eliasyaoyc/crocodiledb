...
use std::path::Path;
use crate::IResult;
use crate::storage::{Storage, write_string_to_file};
...
pub fn update_current<S: Storage>(env: &S, dir: &str, manifest_file_num: u64) -> IResult<()> {
    ...
    let result = write_string_to_file(env, manifest, &tmp, true);
    match &result {

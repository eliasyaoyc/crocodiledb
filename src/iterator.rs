...
impl<I: Iter, F: DerivedIterFactory> ConcatenateIterator<I, F> {
    ...
    #[inline]
    fn maybe_save_err(old: &mut Option<Error>, new: IResult<()>) {
        if old.is_none() {
            if let Err(e) = new {
                *old = Some(e);
            }
...
pub trait KMergeCore {
    ...
    fn smaller<'a>(&self, smallest: &mut Option<&'a [u8]>, iter: &'a dyn Iter) -> bool {
        if iter.valid()
            && (smallest.is_none()
    ...
    /// Updates the smallest if given `iter` has a smaller value and returns true.
    /// Otherwise returns false.
    fn larger<'a>(&self, largest: &mut Option<&'a [u8]>, iter: &'a dyn Iter) -> bool {
        if iter.valid()
            && (largest.is_none()
...
mod tests {
    ...
    struct SimpleKMerger<I: Iter, C: Comparator> {
        cmp: C,
        children: Vec<I>,
    }

    impl<I: Iter, C: Comparator> KMergeCore for SimpleKMerger<I, C> {
        type Cmp = C;
        fn cmp(&self) -> &Self::Cmp {
            &self.cmp
        }
        ...
        fn get_child(&self, i: usize) -> &dyn Iter {
            self.children.get(i).unwrap() as &dyn Iter
        }

        fn get_child_mut(&mut self, i: usize) -> &mut dyn Iter {
            self.children.get_mut(i).unwrap() as &mut dyn Iter
        }
                ...
                F: FnMut(&mut dyn Iter),
        {
            self.children
                .iter_mut()
                .for_each(|i| f(i as &mut dyn Iter));
        }
    ...
    struct SortedIterTestSuite<O: Iter, S: Iter> {
        origin: O, // A sorted array based iterator
        shadow: S, // The iterator to be tested
    }

    impl<O: Iter, S: Iter> SortedIterTestSuite<O, S> {
    ...
    impl<O: Iter, S: Iter> Iter for SortedIterTestSuite<O, S> {
        fn valid(&self) -> bool {
    ...
    impl Iter for TestSimpleArrayIter {
        fn valid(&self) -> bool {

...
pub trait DB {
    ...
    /// the key.
    fn get(&self, write_opt: WriteOptions, key: &[u8]) -> IResult<Option<Vec<u8>>>;
...
impl<S: Storage + Clone, C: Comparator + 'static> DB for CrocodileDB<S, C> {
    ...
    fn get(&self, read_options: ReadOptions, key: &[u8]) -> IResult<Option<Vec<u8>>> {
        self.inner.get(read_options, key)
    }
    ...
    fn write(&self, write_opt: WriteOptions, batch: WriteBatch) -> IResult<()> {
        self.inner.schedule_batch_and_wait(write_opt, batch, false)
    }

    fn close(&self) -> IResult<()> {
        if self.inner.is_shutting_down.load(Acquire) {
            return Ok(());
        }
        self.inner.is_shutting_down.store(true, Relaxed);
...
impl<S: Storage + Clone + 'static, C: Comparator + 'static> DBImpl<S, C> {
    ...
    fn do_compaction(&self, mut c: Compaction<S::F, C>) -> IResult<MutexGuard<VersionSet<S, C>>> {
        let now = Instant::now();
        let mut input_iter = c.new_input_iterator(self.internal_compactor.clone(), self.table_cache.clone())?;
        ...
        while input_iter.valid() && !self.is_shutting_down.load(Ordering::Acquire) {
            ...
            if c.should_stop_before(ikey, &self.internal_compactor) && c.builder.is_some() {
                self.finish_output_file(&mut c, iter_status)?
            }
            let mut drop = false;
            let ucmp = &self.internal_compactor.comparator;
            match ParsedInternalKey::parse_internal_key(ikey) {
                Some(key) => {
                    if current_ukey.is_none() ||
                        ucmp.compare(key.user_key, current_ukey.as_ref().unwrap()) != std::cmp::Ordering::Equal
                    {
                    ...
                    if last_sequence_for_key <= c.oldest_snapshot_alive
                        || (key.value_type == ValueType::KTypeDeletion
                        && key.sequence_number <= c.oldest_snapshot_alive
                    ...
                    last_sequence_for_key = key.sequence_number;
                    if !drop {
                        // Open output file if necessary.
                        if c.builder.is_none() {
                            self.versions.lock().unwrap().create_compaction_output_file(&mut c)?;
                        }
                        ...
                        // Rotate a new output file it the current one is big enough.
                        if builder.file_size() >= self.options.max_file_size as u64 {
...
mod tests {
    ...
    fn new_test_options(o: TestOption) -> Options<BytewiseComparator> {
        let opt = match o {
            ...
            TestOption::UnCompressed => {
                let mut o = Options::default();
                o.compression = CompressionType::KNoCompression;
                o
            }
        };
        opt
    ...
    struct DBTest {
        ...
        db: CrocodileDB<MemoryStorage, BytewiseComparator>,
    }
    ...
    impl DBTest {
        ...
        fn put(&self, k: &str, v: &str) -> IResult<()> {
            self.db
                .put(WriteOptions::default(), k.as_bytes(), v.as_bytes())
        }

        fn delete(&self, k: &str) -> IResult<()> {
        ...
        fn delete_one_sst_file(&self) -> IResult<bool> {
    ...
    fn test_custom_comparator() {
        ...
        impl Comparator for NumberComparator {
            ...
            fn find_shortest_separator(&self, _a: &[u8], b: &[u8]) -> Vec<u8> {
                b.to_vec()
            }
            fn find_short_successor(&self, key: &[u8]) -> Vec<u8> {
                key.to_vec()

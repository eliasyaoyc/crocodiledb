...
use crate::{HashSet, IResult};
use crate::compaction::{base_range, Compaction, CompactionInputs, CompactionReason, CompactionStats, total_range};
use crate::db::build_table;
use crate::db::filename::{FileType, generate_filename, parse_filename, update_current};
use crate::error::Error;
use crate::iterator::{ConcatenateIterator, Iter, KMergeIter};
use crate::opt::{Options, ReadOptions};
use crate::snapshot::{Snapshot, SnapshotList};
use crate::sstable::{FileIterFactory, SSTableIters, TableBuilder};
...
impl<'a, C: Comparator + 'static> VersionBuilder<'a, C> {
    pub fn new(max_levels: usize, base: &'a Version<C>) -> Self {
        ...
        for _ in 0..max_levels {
            levels.push(LevelDiff { delete_files: HashSet::default(), added_files: vec![] })
        }
        Self { levels, base }
    }

    /// Add the given `FileDelta` for later applying `vset.compaction_pointers` will be updated
    pub fn apply<S: Storage + Clone>(
        &mut self,
        delta: FileDelta,
        vset: &mut VersionSet<S, C>)
    {
        ...
        for (level, deleted_file) in delta.deleted_files {
            self.levels[level].delete_files.insert(deleted_file);
        }
        for (level, new_file) in delta.new_files {
            ...
            self.levels[level].delete_files.remove(&new_file.number);
    ...
    fn save_to(self, icmp: &InternalKeyComparator<C>) -> Version<C> {
        let mut v = Version::new(self.base.options.clone(), icmp.clone());
        v.vnum = self.base.vnum + 1;
        for (level, (base_files, delta)) in self.base.files.clone().into_iter().zip(self.levels).enumerate() {
            for f in base_files {
                if !delta.delete_files.contains(&f.number) {
    ...
    fn has_overlapping(icmp: &InternalKeyComparator<C>, files: &[Arc<FileMetaData>]) -> bool {
        for fs in files.windows(2) {
            if icmp.compare(fs[0].largest.data(), fs[1].smallest.data()) != Ordering::Less {
                return true;
            }
        }
        false
...
pub struct VersionSet<S: Storage + Clone, C: Comparator> {
    ...
    // Set of table files to protect then from deletion because they are part of ongoing compaction.
...
impl<S: Storage + Clone + 'static, C: Comparator + 'static> VersionSet<S, C> {
    ...
    {
        let versions = self.current();
        let mut level0 = vec![];
        // Merge all level zero files together since they may overlap.
        for file in versions.files[0].iter() {
        ...
        for files in versions.files.iter().skip(1) {
            if !files.is_empty() {
                let level_file_iter = LevelFileNumIterator::new(self.icmp.clone(), files.clone());
                let factory = FileIterFactory::new(self.icmp.clone(), read_opt, table_cache.clone());
    ...
    /// is both saved to persistent stat and installed as the new current version
    ///
    /// Only Called in situations below:
    ///     1. After minor compaction.
    ///     2. After trivial compaction(only file move)
    ///     3. After major compaction.
    pub fn log_and_apply(&mut self, mut edit: VersionEdit) -> IResult<()> {
        let (v, encoded_edit) = {
            ...
            if let Some(target_log) = edit.log_number {
                assert!(target_log >= self.log_number && target_log < self.next_file_number,
                        "[VersionSet] applying VersionEdit use a invalid log number {}, expect to be at[{},{})",
                        target_log, self.log_number, self.next_file_number
                )
            } else {
            ...
            builder.apply(edit.file_delta, self);
            let mut v = builder.save_to(&self.icmp);
            v.finalize();
        ...
        if self.manifest_writer.is_none() {
            new_manifest_file = generate_filename(&self.db_path, FileType::Manifest, self.manifest_file_number);
            ...
            match self.write_snapshot(&mut writer) {
                ...
                Err(_) => { return self.storage.remove(&new_manifest_file); }
            }
        }
    ...
    ) -> Option<Compaction<S::F, C>>
    {
        ...
        if level > 0 {
            ...
            for (i, file) in overlapping_inputs.iter().enumerate() {
                ...
                if total >= version.options.max_file_size as u64 {
    ...
    /// Otherwise returns a `Compaction` that describes the compaction.
    pub fn pick_compaction(&mut self) -> Option<Compaction<S::F, C>> {
        ...
        let mut compaction = {
            if size_compaction {
                ...
                // Pick the first file that comes after compact_ponter[level]
                for file in current.files[level].iter() {
                    if self.compaction_pointer[level].is_empty()
                        || self.icmp.compare(file.largest.data(), self.compaction_pointer[level].data()) == Ordering::Greater
                    {
                ...
                let level = current.file_to_compact_level.load(std::sync::atomic::Ordering::Acquire);
                if level < self.options.max_levels as usize - 1 {
                    let mut compaction = Compaction::new(self.options.clone(), level, CompactionReason::SeekLimit);
        ...
        if compaction.level == 0 {
            let (smallest, largest) = base_range(&compaction.inputs.base, compaction.level, &self.icmp);
            // Note that the next call will discard the file we placed in
            // inputs[0] earlier and replace it with an overlapping set
            // which will include the picked file.
            compaction.inputs.base = current.get_overlapping_inputs(compaction.level, Some(smallest), Some(largest));
    ...
    /// If `into_base` is true, the file could be pushed into level1 or level2 if there's no too mush overlapping.
    pub fn write_level0_files(
        &mut self,
        db_path: &str,
        table_cache: &TableCache<S, C>,
        mem_iter: &mut dyn Iter,
    ...
    pub(crate) fn create_compaction_output_file(
        ...
        c.builder = Some(crate::sstable::TableBuilder::new(file, self.icmp.clone(), &self.options));
    ...
    pub fn recover(&mut self) -> IResult<bool> {
        ...
        let (current_manifest, file_name) = match String::from_utf8(buf) {
            Ok(s) => {
                if s.is_empty() {
                    return Err(Error::Corruption("CURRENT file is empty"));
                }
            ...
            Err(e) => {
                return Err(Error::CorruptionString(format!(
        ...
        while reader.read_record(&mut buf) {
            ...
            if let Some(ref cmp_name) = edit.comparator_name {
                if cmp_name.as_str() != self.icmp.comparator.name() {
        ...
        if !has_next_file_number {
            return Err(Error::Corruption(
                "no meta-nextfile entry in manifest",
            ));
        }
        if !has_log_number {
            return Err(Error::Corruption(
                "no meta-lognumber entry in manifest",
            ));
        }
        if !has_last_sequence {
            return Err(Error::Corruption(
                "no last-sequence-number entry in manifest",
            ));
        }
        ...
        let mut new_v = builder.save_to(&self.icmp);
        new_v.finalize();
    ...
    /// Remove all the old version.
    /// NOTICE: This func always keeps the last element in `Versions`.
    fn gc(&mut self) {
        let mut i = 0;
    ...
    fn write_snapshot(&self, writer: &mut Writer<S::F>) -> IResult<()> {
        let mut edit = VersionEdit::new(self.options.max_levels);
        // Save metadata
        edit.set_comparator_name(String::from(self.icmp.comparator.name()));
    ...
    /// The input files in `c.level` might expand because of getting a large key range
    /// from newly picked files in `c.level + 1`. And the final key range in `c.level + 1`
    /// should be a subset of `c.level`.
    ...
    fn should_reuse_manifest(&mut self, manifest_file: &str, file_size: u64) -> bool {
        ...
        if let Some((file_type, file_number)) = parse_filename(manifest_file) {
            if file_type != FileType::Manifest || file_size > self.options.max_file_size as u64 {
...
fn add_boundary_inputs_for_compact_files<C: Comparator>(
    ...
    if !files_to_compact.is_empty() {
        ...
        for f in files_to_compact.iter().skip(1) {
            if icmp.compare(f.largest.data(), tmp.largest.data()) == Ordering::Greater {
                tmp = f;
            }
        }
    ...
    for f in level_files {
            ...
            && ucmp.compare(f.smallest.user_key(), largest_key.user_key()) == Ordering::Equal
        {
            match &smallest_boundary_file {
                ...
                Some(current) => {
                    if icmp.compare(f.smallest.data(), current.smallest.data()) == Ordering::Less
                    {
...
mod add_boundary_tests {
    ...
    fn test_empty_level_files() {
        ...
        let f = t.new_file(
            1,
            InternalKey::new(b"100", 2, ValueType::KTypeValue),
            InternalKey::new(b"100", 1, ValueType::KTypeValue),
        );
    ...
    fn test_no_boundary_files() {
        ...
        let f2 = t.new_file(
            1,
            InternalKey::new(b"200", 2, ValueType::KTypeValue),
            InternalKey::new(b"200", 1, ValueType::KTypeValue),
        );
        let f3 = t.new_file(
            1,
            InternalKey::new(b"300", 2, ValueType::KTypeValue),
            InternalKey::new(b"300", 1, ValueType::KTypeValue),
        );
    ...
    fn test_one_boundary_file() {
        ...
        let f1 = t.new_file(
            1,
            InternalKey::new(b"100", 3, ValueType::KTypeValue),
            InternalKey::new(b"100", 2, ValueType::KTypeValue),
        );
        let f2 = t.new_file(
            1,
            InternalKey::new(b"100", 1, ValueType::KTypeValue),
            InternalKey::new(b"200", 3, ValueType::KTypeValue),
        );
        let f3 = t.new_file(
            1,
            InternalKey::new(b"300", 2, ValueType::KTypeValue),
    ...
    fn test_two_boundary_files() {
        ...
        let f1 = t.new_file(
            1,
            InternalKey::new(b"100", 6, ValueType::KTypeValue),
            InternalKey::new(b"100", 5, ValueType::KTypeValue),
        );
        let f2 = t.new_file(
            1,
            InternalKey::new(b"100", 2, ValueType::KTypeValue),
        ...
        let f3 = t.new_file(
            1,
            InternalKey::new(b"100", 4, ValueType::KTypeValue),
            InternalKey::new(b"100", 3, ValueType::KTypeValue),
        );
        t.level_files.push(f2.clone());
        t.level_files.push(f3.clone());
    ...
    fn new_test_file_meta_data(number: u64) -> FileMetaData {
        FileMetaData {
            ...
            file_size: 0,
            number,
            smallest: InternalKey::new(number.to_string().as_bytes(), 1, ValueType::KTypeValue),
            largest: InternalKey::new(number.to_string().as_bytes(), 2, ValueType::KTypeValue),
        }
    }
    ...
    #[test]
    fn test_version_builder_apply_and_save() {
        let opts = Arc::new(Options::<BytewiseComparator>::default());
        let mut mock_vset = VersionSet::new("test".to_owned(), opts.clone(), MemoryStorage::default());
        for (base, diffs, expect) in vec![
            ...
            for (delete, add) in diffs {
                ...
                vb.apply(d, &mut mock_vset);
            }
            let new_v = vb.save_to(&v.icmp);

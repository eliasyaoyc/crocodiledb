...
use crate::db::format::{InternalKey, InternalKeyComparator, LookupKey, MAX_KEY_SEQUENCE, ParsedInternalKey, VALUE_TYPE_FOR_SEEK, ValueType};
...
/// The latest data state after each compact is defined as `Version`.That is,
/// the current DB meta information and the set of sstable with the latest
/// data state at each level.
/// In-memory DBs are written to level-0 tables, and compactions migrate data from
/// level N to level N+1. The tables map internal keys(which are a user key, a delete
/// or set bit, and a sequence number) to user values.
///
/// The tables at level 0 are sorted by increasing fileNum. If two level 0 tables have
/// fileNums i and j and i < j, then the sequence numbers of every internal key in table
/// i are all less than those for table j. The range of internal keys [smallest, largest]
/// in each level 0 table may overlap.
///
/// The tables at any non-0 level are sorted by their internal key range and any two tables
/// at the same non-0 level do not overlap.
///
/// The internal key ranges of two tables at different levels X and Y may overlap, for any X != Y.
///
/// Finally, for every internal key in a table at level X, there is no internal key in a higher
/// level table that has both the same user key and a higher sequence number.
pub struct Version<C: Comparator> {
    ...
    // These fields are initialized `finalize`.
    compaction_score: f32,
    compaction_level: usize,
}

impl<C: Comparator> std::fmt::Debug for Version<C> {
...
impl<C: Comparator + 'static> Version<C> {
    ...
    /// Search the value by the given key in sstable level by level.
    pub fn get<S: Storage + Clone>(
        &self,
        options: ReadOptions,
        key: LookupKey,
        table_cache: &TableCache<S, C>,
    ) -> IResult<(Option<Vec<u8>>, Option<SeekStats>)> {
        let internal_key = key.internal_key();
        let user_key = key.user_key();
        let user_cmp = &self.icmp.comparator;
        ...
        for (level, files) in self.files.iter().enumerate() {
            ...
            if level == 0 {
                ...
                for f in files.iter().rev() {
                    if user_cmp.compare(user_key, f.largest.user_key()) != Ordering::Greater
                        && user_cmp.compare(user_key, f.smallest.user_key()) != Ordering::Less
                    {
                ...
                if index >= files.len() {
                    ...
                    // so let's check the smallest user_key. We use `user_key` because of the given `LookupKey`
                    // may contains the same `user_key` as the `smallest` but a bigger `seq` number than it,
                    // in a comparison by `icmp`.
                    if user_cmp.compare(user_key, target.smallest.user_key()) != Ordering::Less
        ...
        for (file, level) in files_to_seek {
            ...
            match table_cache.get(
                ...
                Some(block_iter) => {
                    ...
                    match ParsedInternalKey::parse_internal_key(encoded_key) {
                        None => return Err(Error::Corruption("bad internal key")),
                        Some(parsed_key) => {
                            if self.options.comparator.compare(parsed_key.user_key, key.user_key()) == Ordering::Equal {
    ...
    pub fn update_stats(&self, stats: Option<SeekStats>) -> bool {
        if let Some(ss) = stats {
            let old = ss.file.allowed_seeks.fetch_update(
            ...
            if file_to_compact.is_none() && old == 1 {
                *file_to_compact = Some(ss.file);
                self.file_to_compact_level.store(ss.level, std::sync::atomic::Ordering::Release);
                return true;
            }
        }
        false
    ...
    /// Returns a String includes number of files in every level.
    ...
    {
        let mut level = 0;
        if !self.overlap_in_level(level, Some(smallest_user_key), Some(largest_user_key)) {
            // No overlapping in level 0
            // we might directly push files to next level if there is no overlap in next level.
            let smallest_internal_key = InternalKey::new(smallest_user_key, MAX_KEY_SEQUENCE, VALUE_TYPE_FOR_SEEK);
            let largest_internal_key = InternalKey::new(largest_user_key, 0, ValueType::KTypeDeletion);
            while level < self.options.max_mem_compact_level {
                ...
                if level + 2 < self.options.max_mem_compact_level {
                    // Check that file dose not overlap too many grandparent bytes.
                    let overlaps = self.get_overlapping_inputs(
                        level + 2,
                        Some(&smallest_internal_key),
                        Some(&largest_internal_key),
                    );
    ...
    pub fn for_each_overlapping(
        ...
        for (level, files) in self.files.iter().enumerate() {
            if level == 0 {
                ...
                for f in files.iter() {
                    if user_cmp.compare(user_key, f.smallest.user_key()) != Ordering::Less
                        && user_cmp.compare(user_key, f.largest.user_key()) != Ordering::Greater
                    {
                        target_files.push(f);
                    }
                }
                ...
                if index >= files.len() {
                    ...
                    if user_cmp.compare(user_key, target.smallest.data()) != Ordering::Less
                        && !func(level, target)
                    {
                        return;
                    }
    ...
    pub fn record_read_sample(&self, internal_key: &[u8]) -> bool {
        if let Some(pkey) = ParsedInternalKey::parse_internal_key(internal_key) {
    ...
    /// `smallest_user_key` is `None` represents a key smaller than all the DB's keys.
    /// `largest_user_key` is `None` represents a key largest than all the DB's keys.
    ...
    /// Notice that both `begin` and `end` is `InternalKey`
    /// but we just compare the user key directly,
        ...
        'outer: loop {
            for file in self.files[level].iter() {
                ...
                if user_begin.is_some()
                    && cmp.compare(file_end, user_begin.unwrap()) == Ordering::Less
                {
                    // `file` is completely before the specified range; skip it.
                    continue;
                }
                if user_end.is_some()
                    && cmp.compare(file_begin, user_end.unwrap()) == Ordering::Greater
                {
                ...
                if level == 0 {
                    ...
                    // added file has expanded the range. If so, restart search to make
                    // sure that we includes all overlapping level 0 files.
                    if user_begin.is_some()
                        && cmp.compare(file_begin, user_begin.unwrap()) == Ordering::Less
                    {
                    ...
                    if user_end.is_some()
                        && cmp.compare(file_end, user_end.unwrap()) == Ordering::Greater
                    {
...
/// Binary search given files to find earliest index of whose largest internal_key >= given key.
...
fn find_file<C: Comparator>(
    ...
    while left < right {
        ...
        let f = &files[mid];
        if icmp.compare(f.largest.data(), ikey) == Ordering::Less {
...
) -> bool
{
    if !disjoint {
        for file in files {
            if key_is_after_file(icmp, file, smallest_user_key)
                || key_is_before_file(icmp, file, largest_user_key) {
                continue;
            } else {
                return true;
            }
    ...
    let index = {
        if let Some(s_user_key) = smallest_user_key {
            let smallest_ikey = InternalKey::new(s_user_key, MAX_KEY_SEQUENCE, VALUE_TYPE_FOR_SEEK);
    ...
    !key_is_before_file(icmp, &files[index], largest_user_key)
}
...
pub const FILE_META_LENGTH: usize = 2 * std::mem::size_of::<u64>();
...
impl<C: Comparator + 'static> Iter for LevelFileNumIterator<C> {
    fn valid(&self) -> bool {
...
mod find_file_tests {
    ...
    impl FindFileTest {
        fn add(&mut self, smallest: &str, largest: &str) {
            ...
            file.smallest = InternalKey::new(smallest.as_bytes(), 100, ValueType::KTypeValue);
            file.largest = InternalKey::new(largest.as_bytes(), 100, ValueType::KTypeValue);
        ...
        fn add_with_seq(&mut self, smallest: (&str, u64), largest: (&str, u64)) {
            let mut file = FileMetaData::default();
            file.number = self.files.len() as u64 + 1;
            file.smallest = InternalKey::new(smallest.0.as_bytes(), smallest.1, ValueType::KTypeValue);
            file.largest = InternalKey::new(largest.0.as_bytes(), largest.1, ValueType::KTypeValue);
            self.files.push(Arc::new(file));
        }

        fn find(&self, key: &str) -> usize {
            let ikey = InternalKey::new(key.as_bytes(), 100, ValueType::KTypeValue);

...
/// An in memory file system based on a simple HashMap with fault injection abilities.
/// Any newly created file or directly will be stored by enum `Node`.
///
...
pub struct MemoryStorage {
    ...
    /// Force write to manifest files to fai.
...
impl MemoryStorage {
    /// Check whether the path is good for create a file.
    /// Return `Err` when the parent dir is not exist.
    fn is_ok_to_create<P: AsRef<Path>>(&self, name: P) -> IResult<()> {
        if let Some(p) = name.as_ref().parent() {
            if !self.is_exist_dir(p) {
                return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{:?}: No directory or file exist", p))));
            }
            if let Some(n) = self
                .inner
                .read()
                ...
                .get(name.as_ref().to_str().unwrap())
            {
                if n.is_file() {
                    ...
                    // Exist dir.
                    return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::AlreadyExists, format!("{:?}: File exists", p))));
                }
            }
            Ok(())
        } else {
            // root file
            Err(Error::IO(std::io::Error::new(std::io::ErrorKind::AlreadyExists, "Unable to create root.")))
        }
    }
...
impl Storage for MemoryStorage {
    ...
    fn create<P: AsRef<Path>>(&self, name: P) -> IResult<Self::F> {
        if self.non_writable.load(Ordering::Acquire) {
            return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, "simulate non writable error")));
        }
        let path = clean(name);
        ...
        file_node.count_random_read = Arc::new(AtomicBool::new(self.count_random_reads));
        ...
        match self.inner.write().unwrap().entry(name) {
            Entry::Occupied(n) => match n.get() {
                ...
                Node::Dir => {
                    return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, format!("{} is a directory", n.key()))));
                }
            }
    ...
    fn open<P: AsRef<Path>>(&self, name: P) -> IResult<Self::F> {
        ...
        match self.inner.read().unwrap().get(&path) {
            Some(n) => match n {
                Node::Dir => Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: try to open a directory", &path)))),
                Node::File(f) => Ok(f.clone()),
            }
            None => Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such file", &path)))),
        }
    }
    ...
    fn remove<P: AsRef<Path>>(&self, name: P) -> IResult<()> {
        ...
        if let Some(n) = map.get(&key) {
            match n {
                Node::Dir => Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: Not such file.", &key)))),
                Node::File(_) => {
                    map.remove(&key);
                    Ok(())
                }
            ...
            Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such file.", &key))))
        }
    }
    ...
    fn remove_dir<P: AsRef<Path>>(&self, dir: P, recursively: bool) -> IResult<()> {
        ...
        let mut map = self.inner.write().unwrap();
        if recursively {
            // Remove the dir and all ites contents.
            let mut to_delete = if let Some(n) = map.get(&key) {
                if n.is_dir() {
                    ...
                    return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such directory.", &key))));
                }
            } else {
                return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such directory.", &key))));
            };
                    ...
                    Node::Dir => {
                        ...
                        for k in map.keys().filter(|k| *k != &key) {
                            if k.starts_with(&key) {
                                return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: is not an empty dir.", &key))));
                            }
                        }
                        map.remove(&key);
                        Ok(())
                    }
                    Node::File(_) => Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: is a file not dir.", &key))))
                }
            } else {
                Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: is a file not dir", &key))))
            }
        }
    }
    ...
    fn list<P: AsRef<Path>>(&self, dir: P) -> IResult<Vec<PathBuf>> {
        ...
        if !map.contains_key(&path) {
            return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such directory", &path))));
        }
        Ok(map
            .keys()
    ...
    fn rename<P: AsRef<Path>>(&self, src: P, target: P) -> IResult<()> {
        let old = clean(src).to_str().unwrap().to_owned();
        if old == MAIN_SEPARATOR.to_string() {
            return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Unable to rename the root.")));
        }
        let new = clean(target).to_str().unwrap().to_owned();
        let mut map = self.inner.write().unwrap();
        match map.remove(&old) {
            ...
            None => Err(Error::IO(std::io::Error::new(std::io::ErrorKind::NotFound, format!("{}: No such file or directory", old))))
        }
    }
...
/// The `File` abstraction for `MemoryStorage` with fault injection ability.
...
pub struct FileNode {
    ...
    count_random_read: Arc<AtomicBool>,
...
impl File for FileNode {
    fn lock_file(&self) -> IResult<()> {
        self.inner.read().unwrap().lock_file()
    }

    fn unlock_file(&self) -> IResult<()> {
        self.inner.read().unwrap().unlock_file()
    }
    ...
    fn write(&mut self, buf: &[u8]) -> IResult<usize> {
        if self.is_manifest() && self.manifest_write_error.load(Ordering::Acquire) {
            return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, "simulated writer error")));
        }
    ...
    fn flush(&mut self) -> IResult<()> {
        if self.is_manifest() {
            if self.manifest_sync_error.load(Ordering::Acquire) {
                Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, "simulated sync error")))
            } else {
                self.inner.write().unwrap().flush()
            }
        } else if self.data_sync_error.load(Ordering::Acquire) {
            Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, "simulated writer error")))
...
impl File for InmemFile {
    fn lock_file(&self) -> IResult<()> {
        if self.lock.load(Ordering::Acquire) {
            Err(Error::IO(std::io::Error::new(std::io::ErrorKind::Other, "Already locked")))
        } else {
    ...
    fn read_at(&self, buf: &mut [u8], offset: u64) -> IResult<usize> {
        if buf.is_empty() {
            ...
            let exact = if buf.len() as u64 + offset > length {
                return Err(Error::IO(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "EOF")));
            } else {
                buf.len()
            };
    ...
    fn read_all(&mut self, buf: &mut Vec<u8>) -> IResult<usize> {
        ...
        match self.contents.read_to_end(buf) {
            Ok(n) => Ok(n),
            Err(e) => Err(Error::IO(e)),
        }
    }
...
mod tests {
    ...
    fn test_mem_file_lock_unlock() {
        ...
        f.lock_file().unwrap();
        assert_eq!(
            f.lock_file().unwrap_err().to_string(),

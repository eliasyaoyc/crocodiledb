//! In leveldb, all characters are string because it can effectively reduce memory usage.
//! But in rust, String bottom layer uses Vec, so i replaced string with vec all together.
//! Notice: you can run `test_string_vec` and view the memory usage.
//!
//! All of number type is little-endian that means stored in the order of first low and then high.
...
pub fn encode_fixed_32(dst: &mut [u8], value: u32) {
    assert!(
        dst.len() >= 4,
        "the length of 'dst' must be at least for a u32, but got {}",
        dst.len()
    );
    unsafe {
...
pub fn decode_fixed_32(source: &[u8]) -> u32 {
    ...
    if source.len() >= 4 {
        unsafe {
            copy_nonoverlapping(source.as_ptr(), &mut data as *mut u32 as *mut u8, 4);
        }
    } else {
...
pub fn decode_fixed_64(source: &[u8]) -> u64 {
    ...
    if source.len() >= 8 {
        unsafe {
            copy_nonoverlapping(source.as_ptr(), &mut data as *mut u64 as *mut u8, 8);
        }
    } else {
        for (i, b) in source.iter().enumerate() {
            data += (u64::from(*b) << (i * 8));

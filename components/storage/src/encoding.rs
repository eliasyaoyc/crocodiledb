...
pub fn decode_boolean(byte: u8) -> Result<bool> {
    match byte {
        0x00 => Ok(false),
        0x01 => Ok(true),
        b => Err(Error::InternalErr(format!("Invalid boolean value {:?}", b))),
    }
}
...
pub fn take_byte(bytes: &mut &[u8]) -> Result<u8> {
    if bytes.is_empty() {
        return Err(Error::InternalErr("Unexpected end of bytes".into()));
    }
    let b = bytes[0];
    *bytes = &bytes[1..];
    Ok(b)
    ...
    let taken = loop {
        match iter.next().map(|(_, b)| b) {
            Some(0x00) => match iter.next() {
                ...
                Some((_, b)) => {
                    return Err(Error::InternalErr(format!("Invalid byte escape {:?}", b)))
                }
                None => return Err(Error::InternalErr("Unexpected end of bytes".into())),
            },
...
pub fn take_value(bytes: &mut &[u8]) -> Result<Value> {
    match take_byte(bytes)? {
        ...
        n => Err(Error::InternalErr(format!("Invalid value prefix {:x?}", n))),
    }
}

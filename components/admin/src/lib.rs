...
use crocodiledb_components_tracing::Handle as TracingHandle;
use crocodiledb_components_metrics::{
    Serve as MetricsServe,
    prom::FmtMetrics,
};

#[derive(Clone)]
pub struct Admin<M> {
    metrics: MetricsServe<M>,
...
impl<M> Admin<M> {
    pub fn new(
        ...
        Self {
            metrics: MetricsServe::new(metrics),
            tracing,
            ready,
            shutdown_tx,
        }

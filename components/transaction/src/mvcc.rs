...
use crocodiledb_components_storage::encoding::{
    encode_bytes, encode_u64, take_byte, take_bytes, take_u64,
};
use crocodiledb_components_storage::Storage;
...
impl MVCC {
    ...
    pub fn status(&self) -> Result<Status> {
        ...
        return Ok(Status {
            ...
            txns_active: store
                .scan(crocodiledb_components_storage::Range::from(
...
impl Transaction {
    ...
    pub fn rollback(self) -> Result<()> {
        ...
        if self.mode.mutable() {
            ...
            let mut scan = session.scan(crocodiledb_components_storage::Range::from(
    ...
    pub fn scan(
    ...
    ) -> Result<crocodiledb_components_storage::Scan> {
        ...
        let scan = self
            .storage
            .read()?
            .scan(crocodiledb_components_storage::Range::from((start, end)));
        Ok(Box::new(Scan::new(scan, self.snapshot.clone())))
    }

    /// Scans keys under a given prefix.
    pub fn scan_prefix(&self, prefix: &[u8]) -> Result<crocodiledb_components_storage::Scan> {
        if prefix.is_empty() {
...
pub struct Scan {
    ...
    scan: Peekable<crocodiledb_components_storage::Scan>,
    /// Keep track of next_back() seen key, whose previous versions should be ignored.
    next_back_seen: Option<Vec<u8>>,
}

impl Scan {
    fn new(mut scan: crocodiledb_components_storage::Scan, snapshot: Snapshot) -> Self {
